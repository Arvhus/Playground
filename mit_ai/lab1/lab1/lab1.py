...
grandparent_rule = IF(AND('parent (?x) (?y)', 'parent (?y) (?z)'),
                      THEN('grandparent (?x) (?z)', 'grandchild (?z) (?z)'))

# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [same_ident_rule, sibling_rule, brother_rule, sister_rule,
                mother_rule, father_rule, son_rule, daughter_rule, cousin_rule,
                grandparent_rule]                    # fill me in
...
WHAT_I_FOUND_BORING = 'Complicated data structs'
